...
public class ExtractSubNetworkIT {

    @Test
    public void testExtractSubNetwork0() throws Exception {
        
        Path actualOutput = null;
        Path tmpDir = null;
        try {
            actualOutput = Files.createTempFile("test-output-ExtractSubNetwork", ".txt");
            tmpDir = Files.createTempDirectory("test-input-ExtractSubNetwork");
        ...
        String sourcePathFile =IThelper.copyProjectResource("seeds.txt",tmpDir);
        String targetPathFile =IThelper.copyProjectResource("targets.txt",tmpDir);
        IThelper.ProcessResult result = IThelper.runCli(
            "fr.inrae.toulouse.metexplore.met4j_toolbox.networkAnalysis.ExtractSubNetwork",
            "-i",
            inputPathFile,
            "-s",
            sourcePathFile,
