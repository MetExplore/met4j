...
public class SetReferencesIT {
    ...
    @Test
    public void testSetReferences0() throws Exception {
        
        Path actualOutput = null;
        Path tmpDir = null;
        try {
            actualOutput = Files.createTempFile("test-output-SetReferences", ".sbml");
            tmpDir = Files.createTempDirectory("test-input-SetReferences");
        ...
        String tabFile =IThelper.copyProjectResource("refs.tsv",tmpDir);
        IThelper.ProcessResult result = IThelper.runCli(
            "fr.inrae.toulouse.metexplore.met4j_toolbox.attributes.SetReferences",
            "-i",
            sbmlFile,
            "-tab",
            tabFile,
